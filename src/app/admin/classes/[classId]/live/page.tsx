...
export default async function AdminLiveClassPage({
  ...
  const session = await requireContentEditorSession();
  ...
  if (!activeSession) {
    return (
      <div className="space-y-4 rounded-2xl border border-line bg-surface p-6">
        <h1 className="text-2xl font-bold">No Active Live Session</h1>
        <p className="text-sm text-muted">Start a live class first from the class channel dashboard.</p>
        <Link
